...
class KeyboardControl(object):
    def __init__(self, world, start_in_autopilot, route_name="routes.xml", expand_routes=False):
        self.route_name = route_name
        self.expand_routes = expand_routes
    ...
    def transform_point(self, point):
        if isinstance(point, list):
            output = []
            for p in point:
                p_dict = {'x': p.transform.location.x,
                 'y': p.transform.location.y,
                 'z': p.transform.location.z,
                 'pitch': p.transform.rotation.pitch,
                 'roll': p.transform.rotation.roll,
                 'yaw': p.transform.rotation.yaw
                 }

                output.append(p_dict)
        else:
            output = {  'x': point.transform.location.x,
                        'y': point.transform.location.y,
                        'z': point.transform.location.z,
                        'pitch': point.transform.rotation.pitch,
                        'roll': point.transform.rotation.roll,
                        'yaw': point.transform.rotation.yaw
                    }

        return output

    def compute_link(self, start, end):
        list_links = [start, end]

        return list_links

    def save_routes(self, ofilename, list_routes, expand_route=False):
        output_routes = []
        for route in list_routes:
            output_route = []
            start_point = route[0]
            if not expand_route:
                output_route.append(self.transform_point(start_point))

            for i in range(1, len(route)):
                end_point = route[i]
                if expand_route:
                    expanded_points = self.compute_link(start_point, end_point)
                    output_route.extend(self.transform_point(expanded_points))
                else:
                    output_route.append(self.transform_point(end_point))
                start_point = end_point
            output_routes.append(output_route)

        # save current expanded current
        with open(ofilename, 'w+') as fd:
            fd.write("<?xml version=\"1.0\"?>\n")
            fd.write("<routes>\n")
            for idx, route in enumerate(output_routes):
                fd.write("\t<route id=\"{}\" map=\"{}\"> \n".format(idx, self.map.name))
                for wp in route:
                    fd.write("\t\t<waypoint x=\"{}\" y=\"{}\" z=\"{}\"".format(wp['x'], wp['y'], wp['z']))
                    fd.write(" pitch=\"{}\" roll=\"{}\" yaw=\"{}\" " "/>\n".format(wp['pitch'], wp['roll'], wp['yaw']))
                fd.write("\t</route>\n")
    ...
    def parse_events(self, client, world, clock):
        global active_route, list_routes
...
def game_loop(args):
    ...
    try:
        ...
        controller = KeyboardControl(world, args.autopilot, args.route_file, args.expand_routes)
...
def main():
    ...
    argparser.add_argument(
        ...
        type=bool,
        default=False,
        help='Do you want to expand the routes?')
