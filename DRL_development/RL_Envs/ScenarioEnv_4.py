"""
    ...
    Some fix from s2e version of scenario RL training.

    But using challenge_evaluator format of scenario management.

    Consider multiple scenarios, including master scenario traffic scenario and others.
...
# path changed in new version of scenario_runners
...
from srunner.scenarioconfigs.scenario_configuration import ActorConfiguration, ActorConfigurationData
...
if __name__ == '__main__':
    ...
    try:
        TrainingEnv = ScenarioTrainer(ARGUMENTS)
        TrainingEnv.run(ARGUMENTS)
    except Exception as e:
        traceback.print_exc()
        if TrainingEnv:
            TrainingEnv.report_challenge_statistics(ARGUMENTS.filename, ARGUMENTS.show_to_participant)
    finally:
        del TrainingEnv