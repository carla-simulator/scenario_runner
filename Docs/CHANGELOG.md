...
## Latest Changes
### :rocket: New Features
* Add support for Jenkins CI pipelines doing automated testing and docker images creation. The initial job build docker images for Scenario Runner,
tag them with the commit id that triggers the pipeline, and store that images into a shared Elastic Container Registry. 
