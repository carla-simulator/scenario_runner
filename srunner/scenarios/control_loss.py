...
class ControlLoss(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self.reference_waypoint = self._map.get_waypoint(config.trigger_points[0].location)
        self.loc_list = []
        self.obj = []
    ...
    def _initialize_actors(self, config):
        ...
        first_loc, _ = get_location_in_distance_from_wp(self.reference_waypoint, self._distance[0])
        second_loc, _ = get_location_in_distance_from_wp(self.reference_waypoint, self._distance[1])
        third_loc, _ = get_location_in_distance_from_wp(self.reference_waypoint, self._distance[2])

        self.loc_list.extend([first_loc, second_loc, third_loc])
        self._dist_prop = [x - 2 for x in self._distance]

        self.first_loc_prev, _ = get_location_in_distance_from_wp(self.reference_waypoint, self._dist_prop[0])
        self.sec_loc_prev, _ = get_location_in_distance_from_wp(self.reference_waypoint, self._dist_prop[1])
        self.third_loc_prev, _ = get_location_in_distance_from_wp(self.reference_waypoint, self._dist_prop[2])
