...
from srunner.osc2_stdlib.observer import EventListener

from srunner.scenarios.basic_scenario import BasicScenario
from srunner.scenariomanager.timer import GameTime, TimeOut

from agents.navigation.global_route_planner import GlobalRoutePlanner
from srunner.scenariomanager.scenarioatomics.atomic_trigger_conditions import IfTriggerer, TimeOfWaitComparison
from srunner.scenariomanager.scenarioatomics.atomic_behaviors import ActorTransformSetter, ChangeTargetSpeed, \
    UniformAcceleration, \
    ChangeWeather, ChangeRoadFriction, Idle, KeepVelocity, LaneChange, StopVehicle, WaypointFollower, \
    calculate_distance, ChangeActorControl, ChangeActorTargetSpeed
...
from srunner.scenariomanager.scenarioatomics.atomic_repeat import SuccessIsRunning
...
from srunner.osc2_stdlib.modifier import ChangeSpeedModifier, LaneModifier, Modifier, ChangeLaneModifier, \
    PositionModifier, SpeedModifier, AccelerationModifier
...
def para_type_str_sequence(config, arguments, line, column, node):
    ...
    if isinstance(arguments, List):
        for arg in arguments:
            if isinstance(arg, Tuple):
                if isinstance(arg[1], int):
                    ...
                    physical_type_name = OSC2Helper.find_physical_type(config.physical_dict,
                                                                       arg[1].unit.physical.si_base_exponent)
        ...
        if isinstance(arguments[1], int):
            ...
            physical_type_name = OSC2Helper.find_physical_type(config.physical_dict,
                                                               arguments[1].unit.physical.si_base_exponent)
        ...
        physical_type_name = OSC2Helper.find_physical_type(config.physical_dict,
                                                           arguments.unit.physical.si_base_exponent)
...
def process_speed_modifier(config, modifiers, duration: float, all_duration: float, father_tree):
    ...
    for modifier in modifiers:
        ...
        if isinstance(modifier, SpeedModifier):
            ...
            uniform_accelerate_speed = UniformAcceleration(actor, current_car_speed, target_velocity,
                                                           accelerate_speed, start_time)
...
def process_location_modifier(config, modifiers, duration: float, father_tree):
    ...
    for modifier in start_group:
        ...
        if location == 'left_of':
            ...
            # Same lane
            pass
        elif location == 'ahead_of' or location == 'behind':
    ...
    for modifier in end_group:
        ...
        if location == 'ahead_of' or location == 'behind':
            ...
            if wp_lists:
                end_wp = wp_lists[0]
        elif location == 'left_of' or location == 'right_of' or location == 'same_as':
    ...
    if end_wp:
        ...
        distance = calculate_distance(current_car_transform.location, end_wp.transform.location, grp)

        car_need_speed = distance / float(duration)

        current_car_conf.set_arg({'desired_speed': car_need_speed})
        LOG_WARNING(f'{npc_name} car desired speed will be set to {car_need_speed * 3.6} km/h')
    ...
    if end_lane_wp:
        ...
        current_car_conf = config.get_car_config(npc_name)
        current_car_transform = current_car_conf.get_arg('init_transform')
        car_lane_wp = CarlaDataProvider.get_map().get_waypoint(current_car_transform.location)
        ...
        if end_lane_wp and car_lane_wp:
            ...
            lane_change = LaneChange(
                car_actor, speed=None, direction=direction, distance_same_lane=5, distance_other_lane=10)
...
class OSC2Scenario(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config: OSC2ScenarioConfiguration, osc2_file, debug_mode=False,
                 criteria_enable=True, timeout=300):
        """
    ...
    class BehaviorInit(ASTVisitor):
        ...
        def visit_do_member(self, node: ast_node.DoMember):
            ...
            for child in node.get_children():
                ...
                if isinstance(child, ast_node.DoMember) or isinstance(child, ast_node.WaitDirective) or isinstance(
            ...
            if sub_node == None:
                ...
                if isinstance(sub_node, ast_node.DoMember):
                    ...
                    raise NotImplementedError(f'no supported ast node')
        ...
        def visit_wait_directive(self, node: ast_node.WaitDirective):
            behaviors = py_trees.composites.Sequence(policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL,
                                                     name="wait")
            subbehavior = py_trees.composites.Sequence(policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL,
                                                       name="behavior")
            ...
            ret = getattr(event.Event(), event_name)(other_car, distance)
        ...
        def visit_emit_directive(self, node: ast_node.EmitDirective):
            ...
            actor = arguments[0][1]
            distance = float(arguments[1][1].gen_physical_value())
            ret = getattr(event.Event(), function_name)(actor, distance)
        ...
        def visit_behavior_invocation(self, node: ast_node.BehaviorInvocation):
            ...
            behavior = py_trees.composites.Parallel(
                ...
                name=behavior_invocation_name + ' duration=' + str(int(self.__duration)))
            ...
            '''
            # ego_vehicle.drive(path) is translated to atomic behavior WaypointFollower
            for child in node.get_children():
            ...
            process_speed_modifier(self.father_ins.config, speed_modifiers, self.__duration,
                                   self.father_ins.all_duration, actor_drive)
        ...
        def visit_range_expression(self, node: ast_node.RangeExpression):
            ...
            range = Range(start_num, end_num)

            if unit_name:
                return Physical(range, unit_name)
            else:
                return range
