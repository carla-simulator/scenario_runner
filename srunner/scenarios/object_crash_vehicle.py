...
class PedestriansCrossingSidewalk(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config,
        ...
        self._adversary_speed = 4.0  # Speed of the adversary [m/s]
        self.first_adversary_speed = 2.0  # Speed of the adversary [m/s]
        self.second_adversary_speed = 2.0  # Speed of the adversary [m/s]
        self.third_adversary_speed = 6.0  # Speed of the adversary [m/s]
        self.fourth_adversary_speed = 3.0  # Speed of the adversary [m/s]
        self._reaction_time = 0.8  # Time the agent has to react to avoid the collision [s]
        self._reaction_ratio = 0.12  # The higehr the number of lane changes, the smaller the reaction time
        self._min_trigger_dist = 7.0#6.0  # Min distance to the collision location that triggers the adversary [m]
        self._ego_end_distance = 40
        self.timeout = timeout

        self._number_of_attempts = 9
    ...
    def _create_behavior(self):
        ...
        # Move the first adversary
        speed_duration = 8.0 * collision_duration
        speed_distance = 8.0 * collision_distance
        sequence.add_child(KeepVelocity(
            self.other_actors[0], self.first_adversary_speed,
            duration=speed_duration, distance=speed_distance, name="FirstAdversaryCrossing"))
        
        # Move the second adversary
        speed_duration = 8.0 * collision_duration
        speed_distance = 8.0 * collision_distance
        sequence.add_child(KeepVelocity(
            self.other_actors[1], self.second_adversary_speed,
            duration=speed_duration, distance=speed_distance, name="SecondAdversaryCrossing"))
        
        # Move the third adversary
        speed_duration = 8.0 * collision_duration
        speed_distance = 8.0 * collision_distance
        sequence.add_child(KeepVelocity(
            self.other_actors[2], self.third_adversary_speed,
            duration=speed_duration, distance=speed_distance, name="ThirdAdversaryCrossing"))
        
        # Move the fourth adversary
        speed_duration = 8.0 * collision_duration
        speed_distance = 8.0 * collision_distance
        sequence.add_child(KeepVelocity(
            self.other_actors[3], self.fourth_adversary_speed,
            duration=speed_duration, distance=speed_distance, name="FourthAdversaryCrossing"))
        
        # Remove everything
        sequence.add_child(ActorDestroy(self.other_actors[0], name="DestroyFirstAdversary"))
        sequence.add_child(ActorDestroy(self.other_actors[1], name="DestroySecondAdversary"))
        sequence.add_child(ActorDestroy(self.other_actors[2], name="DestroyThirdAdversary"))
        sequence.add_child(ActorDestroy(self.other_actors[3], name="DestroyFourthAdversary"))
        sequence.add_child(DriveDistance(self.ego_vehicles[0], self._ego_end_distance, name="EndCondition"))
