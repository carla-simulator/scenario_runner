...
class PedestrianCrossing(BasicScenario):
    ...
    def _initialize_actors(self, config):
        ...
        while self._number_of_attempts > 0:
            ...
            actors = CarlaDataProvider.request_new_batch_actors(
                self._adversary_type, len(spawn_transforms), spawn_transforms)

            if None in actors or len(actors) < self._adversary_number:
                self._number_of_attempts -= 1
                move_dist = self._retry_dist
                # remove actors
                for actor in actors:
            ...
            # All actors ready, end
            break

        if self._number_of_attempts == 0:
            raise Exception("Couldn't find viable position for the adversary actors")
    ...
    def _create_behavior(self):
        ...
        collision_location = self._collision_wp.transform.location
        collision_distance = collision_location.distance(self._adversary_transform.location)
        collision_duration = collision_distance / self._max_adversary_speed
        reaction_time = self._reaction_time - self._reaction_ratio * self._num_lane_changes
        collision_time_trigger = collision_duration + reaction_time
        ...
        # Move the adversary
        adversary_root = py_trees.composites.Parallel(
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL, name="AdversaryMove")
        speed_duration = 8.0 * collision_duration
        speed_distance = 8.0 * collision_distance

        for i in range(self._adversary_number):
            adversary_root.add_child(KeepVelocity(
                self.other_actors[i], self._adversary_speed_list[i],
                duration=speed_duration, distance=speed_distance, name="AdversaryCrossing"))

        sequence.add_child(adversary_root)

        # Remove everything
        for actor in self.other_actors:
            sequence.add_child(ActorDestroy(actor, name="DestroyAdversary"))
        sequence.add_child(DriveDistance(self.ego_vehicles[0], self._ego_end_distance, name="EndCondition"))
